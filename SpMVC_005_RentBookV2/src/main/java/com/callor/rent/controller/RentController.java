 ...
 * SessionAttributes : Spring Freamwork 차원에서 구현하는 Session
...
//		model.addAttribute("RENT_WORK",rentBookVO);
	...
	public String work_insert(
		...
		/*
		 * SessionAttributes 에 보관중인 객체(데이터를) 모두 사용한 후에는
	...
	public RentBookVO newRentBook() {
		...
		// Java 1.8 이전에는 Date나 Calendar를 사용한다.
		
		// Java 1.8 이후의 날짜, 시간 관련 클래스
		// Java 1.8 이후에서는 Date, Calendar 클래스 보다는 Local** 클래스를
		// Java는 1.8 을 기준으로 이전과 이후를 다른 세대로 구분한다
		// 1.7 이하는 1.x 로 표기 1.8 부터 8, 9, 10 등으로 표기한다
		// Java 1.2 가 탄생했을때도 1.1과 1.2를 다른세대로 구분하여 Java2라고 불렀었다
